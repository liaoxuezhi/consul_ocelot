...
namespace TestClient
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine("请输入a为admin clinet,b为user client,e为退出");
        ...
        static void GetString(HttpClient client)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        case "1":                         
                            result = client.GetStringAsync("http://172.16.11.180:5000/api001/values").GetAwaiter().GetResult();
                            break;
                        case "2":
                            result = client.GetStringAsync("http://172.16.1.180:5000/api002/values").GetAwaiter().GetResult();
                            break;
                        case "e":
                            return;
                    }
